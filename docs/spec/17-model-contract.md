...
3. **Non-blocking.** Whether the two-slot behaviour holds for a third slot, if research ever needs one. The two-slot case is measured: on 2026-09-12, alternating twenty rounds, each slot reused its own prefix - 123 tokens of 128 and 312 of 317 - while the other alternated against it, and a one-byte change collapsed the reuse to 7 and quintupled the prefill time. They do not evict each other. See `experiments/03-model-latency/`.
