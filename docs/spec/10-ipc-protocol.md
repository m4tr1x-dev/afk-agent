...
1. **Non-blocking.** Whether the shell should be able to attach to a core started by another user session. Currently no, and per-user access control assumes that stays so.
