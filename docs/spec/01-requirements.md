...
| `FR-MODEL-004` | The system MUST monitor available graphics memory and degrade — reducing context, then visual budget, then moving the deliberative model off the graphics processor — before the game is starved. The reading MUST come from a platform query against the adapter, not from the inference runtime's own report. | An agent that makes the game stutter is uninstalled regardless of how well it plays. The second sentence is measured rather than stylistic: the runtime's device report under-stated a resident 16 GiB model as 1.2 GiB and did not see a running game at all — `experiments/03-model-latency/`. | Bench, Manual |
