...
| 3 | What are the latency and memory figures at visual budgets of 140 and 1120, at 8K and 16K context, **with a game running**? | Measure. The figure without a game in memory is not the figure that matters — and neither is the figure with a forty-token prompt, nor the one that sends the same frame twice. Three conditions: cache off, cache on with the same image, and cache on with a fresh image, which is the only one that is a tick. | Model contract, performance budgets | **Tactical answered 2026-09-12; deliberative still open.** With a game resident, a product-sized prefix and a fresh frame per request: **445 ms** for E4B and **596 ms** for 26B A4B at a 256-pixel image, against a 150-400 ms target. The target is not met by any variant measured, and the cadence in `05-reasoning-loop.md` is corrected from 1-4 Hz to **1-2 Hz** rather than the measurement being argued with. **`FR-CTX-002` turns out to be load-bearing rather than an optimisation**: at that prompt size the prefix cache is worth **2686 ms per tick** (3282 ms against 596 ms), so without byte-identical blocks 1 and 2 a tactical call costs three and a half seconds. Measured against a forty-token prompt it appears to buy nothing, which is a property of the measurement. Two thirds of a tick is not the image: the text-only floor is 333 ms for E4B. See `experiments/03-model-latency/`. |
...
| 5 | Does the prompt prefix cache behave as assumed with two pinned slots at different visual budgets? | Measure prefix reuse across consecutive ticks on each slot, **two independent ways** - the runtime's counter and the prefill time, because a server can report a hit and prefill anyway. Then change one byte of one slot's prefix and confirm the reuse collapses; without that control, nothing distinguishes a cache from a server reporting whatever it likes. | Model contract | **Yes, 2026-09-12.** Twenty alternating rounds on E4B: slot A reuses 123 tokens of 128 and prefills 5 at 27.3 ms; slot B reuses 312 of 317 and prefills 5 at 37.8 ms. The one-byte control collapses A's reuse to 7 tokens, its prefill to 121, and its time to 137.7 ms - a five-fold jump, so the measurement was measuring a cache. **ADR-0006's one-slot fallback is not needed.** See `experiments/03-model-latency/`. |
| 6 | What is the 26B A4B throughput on the processor alone? | Measure on the reference hardware, in the shape of a real deliberative call - a long answer, not one tool call under a grammar that stops after eight tokens. The criterion was fixed in ADR-0006 before the run: under 60 seconds is a degradation, above it is a hang. | Model residency | **Usable and marginal, 2026-09-12.** A full deliberative call - 1024-pixel frame, 4800-token prefix, answer free-running to 512 tokens - takes **41.5 s** with the prefix cache and **57.5 s** without, against a 60-second criterion. Raw generation is 12.6 tokens per second against 141.8 on the graphics processor. Two consequences the pages now carry: at 41.5 s the fallback sustains only the slow end of the 10-60 second deliberative cadence, so degrading residency also degrades cadence; and `FR-CTX-002` is load-bearing here too, since without it the call clears the criterion by 4%. See `experiments/03-model-latency/`. |
| 7 | Can the capture border be suppressed for an unpackaged application on this Windows build? | Prototype, and prove the instrument can see a border before believing it cannot: capture with the border **required** as the positive control, with it suppressed, and with nothing capturing as a stability control. Read the **screen**, not the captured frame - the border is drawn around the window for the person sitting there and may not appear in a capture at all. | Overlay, capture | **Yes, 2026-09-12, reproduced three times.** With the border required the screen changes 4.54% in a 16 px edge band; with it suppressed, 0.00%; with nothing capturing, 0.00%. Cursor suppression is accepted on every run too, which `FR-PERC-006` needs. Two earlier attempts produced confident wrong answers from instruments that could not see the thing: the first differenced two captures, which cannot show a border that is never captured; the second read the screen at the target's coordinates while another window covered it. The same harness measured the routes - Windows Graphics Capture at **2.27 ms and 55 fps** against 12.92 ms for `PrintWindow` on a windowed game - and three silent failure signatures for `FR-PERC-009`: 100% black frames, 100% repeated frames, and fewer than five frames in eight seconds. The overlay self-exclusion assertion also passed: three runs, 2400 captured frames, **zero pixels** of the overlay's colour - and the positive control changed the question, because a window-scoped capture composites one window and therefore excludes an overlay **by construction**, flag or no flag. A screen-scoped reading sees it plainly (24000 pixels, exactly its size) and `WDA_EXCLUDEFROMCAPTURE` removes every one. Not measured: borderless and exclusive fullscreen, desktop duplication, multiple displays, and the real overlay with a tooltip and a flyout open. See `experiments/07-capture/`. |
