...
| --- | --- | --- | --- | --- |
| 1 | Does synthesised relative mouse movement reach a real game at all? | Build the reachability probe: a minimal program that captures a window, synthesises relative movement, and confirms the camera turned. Run it against five games of different engines. | Everything. If this fails there is no project. | **Three of five, 2026-09-12.** Reached in Breathedge (Unreal Engine 4), Xonotic (DarkPlaces, 0.892 columns per unit at R2 0.957) and AssaultCube (Cube, 1.072 at R2 1.000). An absolute reposition produced no turn in Xonotic and a 245-column swing in AssaultCube, which is a third outcome `06-action-and-input.md` did not allow for and now does. See `experiments/01-reachability/results/`. Two titles outstanding. The wrap test gives AssaultCube **0.0909 degrees per mouse unit**, which agrees with its columns-per-unit figure at a field of view of about 110 degrees — two independent measurements, one cross-check. |
